...
bool InternetProtocolProvider::OnEtherFrameReceived(uint8_t* etherframePayload , uint32_t size){
    ...
    //check if length of ipv4 protocl exceed the size of ethernet frame 
    if(length > size)
        length = size;
...
void InternetProtocolProvider::Send(uint32_t dstIP_BE , uint8_t protocol , uint8_t* data , uint32_t size){
    ...
    message->headerLength = sizeof(InternetProtocolV4Message)/4; //The length of header is stored in form of number of worlds
    message->tos = 0; 
    ...
    message->FlagsAndOffset = 0x0040; //No fragmentation

...
bool TransmissionControlProtocolProvider::OnInternetProtocolReceived(uint32_t srcIP_BE , uint32_t dstIP_BE , 
    ...
    //size of 20 bytes is used here instead of size of
    //TCP header because TCP header contain extra 4 bytes for options field 
    ...
    if(socket != 0 && socket->state != CLOSED){
        switch((msg->flags) & (SYN | ACK | FIN)){
            case SYN:
                //if SYN is sent it will only be proccessed
                //if server is listening if not the connection 
                //will be reset
                ...
                socket->sequenceNumber = 0xbeecafe;
            ...
            case SYN | ACK:
                //We should receive SYN-ACK
                //If we only sent something
            ...
            case ACK:
                ...
                //If the flags contain nothing but ACK
                //then this called piggybacking
                if(msg->flags == ACK)
                break;


            default:
                //we check the sequence number and acknowledgement number
                //if they arenot identical that means that packages is received
                //at different order
                if(bigEndian32(msg->sequenceNumber) == socket->acknowledgementNumber){
                    // the value we get from handler will till us if we should reset the 
                    //connection or not
    ...
    if(reset){
        //We check first if there is a socket
        if(socket != 0){
            Send(socket , 0 , 0 ,RST);
        }else{
    ...
    //removing socket if its state is closed
    if(socket != 0 && socket->state == CLOSED)
        for(uint16_t i = 0 ; i < numSockets && socket == 0 ; i++){
...
void TransmissionControlProtocolProvider::Send(TransmissionControlProtocolSocket* socket , uint8_t* data , uint16_t size , uint16_t flags){
    ...
    uint16_t length_Inc_IP_Hdr = totalLength + sizeof(TransmissionControlProtocolPseudoHeader);

    uint8_t* buffer = (uint8_t*)MemoryManager::activeMemoryManager->malloc(length_Inc_IP_Hdr);
    ...
    phdr->protocol = 0x0600; //protocol number in Big endian 
    phdr->totalLength = ((totalLength & 0xFF00) >> 8) | ((totalLength & 0X00FF) << 8);

    msg->checksum = 0;
    msg->checksum = InternetProtocolProvider::Checksum((uint16_t*)buffer , length_Inc_IP_Hdr);
